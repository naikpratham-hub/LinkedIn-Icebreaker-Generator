...
import { CopyIcon, CheckIcon, StarIcon, EditIcon, SaveIcon, ThumbsUpIcon, ThumbsDownIcon } from './icons';
...
const IcebreakerCard: React.FC<IcebreakerCardProps> = ({ title, text, isPrimary = false }) => {
  ...
  const handleFeedback = (type: 'up' | 'down') => {
    ...
    analyticsService.trackEvent('icebreaker_feedback', {
      cardTitle: title,
      feedback: newFeedback === 'up' ? 'positive' : newFeedback === 'down' ? 'negative' : 'cleared',
      icebreakerText: editedText,
      isPrimary,
    });
  };
  ...
  return (
    <div className={`p-6 rounded-lg shadow-lg relative transition-all duration-300 flex flex-col ${isPrimary ? primaryClasses : defaultClasses}`}>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-brand-gray-700/50 flex items-center justify-end space-x-2">
        <button
          onClick={() => handleFeedback('up')}
          className={`p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-brand-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-blue dark:focus:ring-offset-brand-gray-800 transition-all ${
            feedback === 'up' ? 'text-green-500 bg-green-100 dark:bg-green-900/50' : 'text-gray-400'
          }`}
        ...
        <button
          ...
          className={`p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-brand-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-blue dark:focus:ring-offset-brand-gray-800 transition-all ${
            feedback === 'down' ? 'text-red-500 bg-red-100 dark:bg-red-900/50' : 'text-gray-400'
          }`}
